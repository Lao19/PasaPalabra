...
const App = () => {
  // Lista maestra de todas las preguntas posibles, categorizadas por letra y por jugador.
  // Cada jugador tiene su propio set de preguntas.
  const allPlayersQuestionsData = useMemo(() => ({
    'player1': [ // Preguntas para el Jugador 1 (ej: tu novio)
      { letter: 'A', questions: [
    ...
    'player2': [ // Preguntas para el Jugador 2 (ej: tú)
      { letter: 'A', questions: [
        { question: 'Capital de Francia.', answer: 'PARIS' },
        { question: 'Color del cielo en la noche.', answer: 'AZULOSCURO' },
        { question: 'Instrumento musical de viento con teclado.', answer: 'ACORDEON' }
      ]},
      { letter: 'B', questions: [
        { question: 'Animal que produce miel.', answer: 'ABEJA' },
        { question: 'Ciudad famosa por su torre inclinada.', answer: 'PISA' },
        { question: 'Prenda de vestir que cubre el tronco.', answer: 'CAMISA' }
      ]},
      { letter: 'C', questions: [
        { question: 'Planeta rojo.', answer: 'MARTE' },
        { question: 'Metal precioso de color gris plateado.', answer: 'PLATA' },
        { question: 'Árbol que da bellotas.', answer: 'ENCINA' }
      ]},
      { letter: 'D', questions: [
        { question: 'Número de dedos en una mano.', answer: 'CINCO' },
        { question: 'Deporte de combate de origen japonés.', answer: 'JUDO' },
        { question: 'Sinónimo de regalo.', answer: 'DON' }
      ]},
      { letter: 'E', questions: [
        { question: 'Elemento químico cuyo símbolo es Fe.', answer: 'HIERRO' },
        { question: 'Continente más grande del mundo.', answer: 'ASIA' },
        { question: 'Animal que vive en el desierto y tiene joroba.', answer: 'CAMELLO' }
      ]},
      { letter: 'F', questions: [
        { question: 'Capital de Italia.', answer: 'ROMA' },
        { question: 'Estación del año de las flores.', answer: 'PRIMAVERA' },
        { question: 'Parte del cuerpo que usamos para oler.', answer: 'NARIZ' }
      ]},
      { letter: 'G', questions: [
        { question: 'Color del sol.', answer: 'AMARILLO' },
        { question: 'Animal que relincha.', answer: 'CABALLO' },
        { question: 'Sinónimo de grande.', answer: 'ENORME' }
      ]},
      { letter: 'H', questions: [
        { question: 'Elemento químico cuyo símbolo es O.', answer: 'OXIGENO' },
        { question: 'Lugar donde se guardan los libros.', answer: 'BIBLIOTECA' },
        { question: 'Sinónimo de felicidad.', answer: 'GOZO' }
      ]},
      { letter: 'I', questions: [
        { question: 'País con forma de bota.', answer: 'ITALIA' },
        { question: 'Contrario de exterior.', answer: 'INTERIOR' },
  ...
  // Información de los jugadores para la selección
  const playersInfo = useMemo(() => ({
    'player1': { name: 'Tu Novio', avatar: 'https://placehold.co/100x100/FFD700/000000?text=Novio' },
    'player2': { name: 'Tú', avatar: 'https://placehold.co/100x100/ADD8E6/000000?text=Tu' },
  }), []);
  ...
  const [initialTimeMinutes, setInitialTimeMinutes] = useState(2); // Tiempo inicial en minutos (por defecto 2 minutos)
  const [gamePhase, setGamePhase] = useState('config'); // 'config', 'playerSelection', 'playing', 'finished'
  const [selectedPlayer, setSelectedPlayer] = useState(null); // 'player1' o 'player2'
  ...
  // Función para pasar de la configuración de tiempo a la selección de jugador
  ...
  const startGame = (playerKey) => {
    ...
    const questionsForThisRound = allPlayersQuestionsData[playerKey];

    // Seleccionar una pregunta aleatoria para cada letra para la ronda actual
    const selectedQuestions = questionsForThisRound.map((letterData, index) => {
    ...
    setScoreCorrect(0);
    setScoreIncorrect(0);
    setTimeLeft(initialTimeMinutes * 60); // Convertir minutos a segundos
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-400 to-purple-600 flex flex-col items-center justify-center p-4 font-inter text-white">
      <div className="bg-white bg-opacity-20 backdrop-filter backdrop-blur-lg p-8 rounded-xl shadow-2xl w-full max-w-2xl text-center border border-white border-opacity-30">
        ...
        {/* Fase de Configuración de Tiempo */}
        {gamePhase === 'config' && (
          <div className="flex flex-col items-center">
            <div className="mb-6">
              ...
              <input
                ...
                onChange={(e) => setInitialTimeMinutes(Math.max(1, parseInt(e.target.value) || 1))}
        ...
        {/* Fase de Selección de Jugador */}
        {gamePhase === 'playerSelection' && (
          <div className="flex flex-col items-center">
            <h2 className="text-2xl md:text-3xl font-bold mb-6 text-yellow-300">
              ¿Quién hará las preguntas?
            </h2>
            <div className="flex flex-col md:flex-row gap-8">
              {Object.entries(playersInfo).map(([key, player]) => (
                <div
                  key={key}
                  className="flex flex-col items-center cursor-pointer p-4 rounded-lg hover:bg-white hover:bg-opacity-30 transition duration-200 ease-in-out"
                  onClick={() => startGame(key)}
                >
                  <img
                    src={player.avatar}
                    alt={player.name}
                    className="w-24 h-24 rounded-full border-4 border-yellow-300 shadow-lg mb-3 object-cover"
                  />
        ...
        {/* Fase de Juego (Playing o Finished) */}
          ...
          <>
            ...
            {gameFinished ? (
              <div className="text-2xl font-bold mb-6">
                ...
                <button
                  onClick={() => {
                    setGamePhase('config'); // Volver a la configuración para reiniciar
                    setSelectedPlayer(null); // Limpiar jugador seleccionado
                  }}
